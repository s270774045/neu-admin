...
//		}
		return getPage(pageParam, new Criterion[0]);
	}
	
	@SuppressWarnings("unchecked")
	public Page<T> getPage(PageParam pageParam, Criterion... criterions) {
		// 进行分页查询
		long total = find(criterions).size();
		int skip = pageParam.getStart();
		int limit = pageParam.getLength();
		Criteria query = createCriteria(criterions);
	...
	public FormatedPage getFormatedPage(PageParam pageParam, Criterion... criterions) {
		// 分页查询结果格式化
		return FormatHandler.handle(getPage(pageParam, criterions), pageParam);
	}
}